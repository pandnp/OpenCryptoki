...
int migrate_wrapped_keys(CK_SLOT_ID slot_id, char *userpin, int masterkey)
{
	...
	case MK_SYM:
			...
			break;
	default:
			print_error("unknown key type (%d)\n", key_type);
			return -1;
	}
...
void usage(char *progname)
{
	...
	printf(" -n, --dryrun\t\t\tPerform the steps without actually writing");
	printf(" \n\t\t\t\tthe migrated data\n\n");
	return;
}
...
{
	int ret, opt, c_flag = 0, masterkey = 0;
	int data_store_len;
	...
	int long_index;
	while ((opt = getopt_long(argc, argv, "m:d:s:k:hnv", long_opts, NULL))
		!= -1) {
		switch (opt) {
		case 'd':
